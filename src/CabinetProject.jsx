  ...
  const buildRows = (cabsToUse) => {
    ...
    (cabsToUse || cabs).forEach((cab, cabIdx) => {
      ...
      d.parts.forEach((part) => {
        ...
        // Non-depth axis — used both for the back-panel groove (ranura) and
        // for which single edge gets front edge-banding.
        const nonDepthAxis = ranuraSide(part.aLabel, part.bLabel, part.a, part.b);

        // Ranura only applies when this cabinet actually has a grooved-in thin
        // hardboard back — a full melamine back is screwed on, not grooved in.
